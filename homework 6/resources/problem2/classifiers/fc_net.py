...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #fc layer first).
        soft_loss, upstreamd = softmax_loss(out2, y)
        dx2, dw2, db2 = affine_backward(upstreamd, cache2)
        rback1 = relu_backward(dx2, rcache1)
        dx1, dw1, db1 = affine_backward(rback1, cache1)
        grads['W1'] = dw1
        grads['W2'] = dw2
        grads['b1'] = db1
        grads['b2'] = db2

        loss += (np.sum(self.params['W1']*self.params['W1']) + np.sum(self.params['W2']*self.params['W2']))
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
